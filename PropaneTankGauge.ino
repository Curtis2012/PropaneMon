...
*
*
*   ProapaneTankGauge.ino
*
*
*
			*****		  NOTE: Remember RST pin must be disconnected in order to flash!      *****
...
void sendSample()
{
	...
	{
		connectMQTT(false, mqttTopicData, MDNS.IP(hostEntry));
		delay(3000);
	};
	...
	mqttClient.loop();
	//rc = mqttClient.publish(mqttTopicData, msgbuff);  // send twice, because first send after sleep often fails
	//if (!rc) Serial.print("MQTT publish failed!");
...
void setup()
{
	...
	while (!setupMQTT(MDNS.IP(hostEntry), MDNS.port(hostEntry), false, mqttTopicData, handleMQTTmsg)) {
		delay(5000);
	};
...
void loop()
{
	...
	msgn = sprintf(msgbuff, "Entering deep sleep for %i ms", setSleepTime());
